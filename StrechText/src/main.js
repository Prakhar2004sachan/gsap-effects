...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let i = 0; i < 2; i++) {
    const elem = [textElement1, textElement2];
    ScrollTrigger.create({
      trigger: `.sticky-text-${i + 1}`,
      start: "top bottom",
      end: "top top",
      scrub: 1,
      onUpdate: (self) => {
        const currentScale = targetScales[i] * self.progress;
        setScaleY(elem[i], currentScale);
      },
    });
    ...
    ScrollTrigger.create({
      ...
      scrub: 1,
      onUpdate: (self) => {
        const currentScale = targetScales[i] * (1 - self.progress);
        setScaleY(elem[i], currentScale);
      },
    });
  }
